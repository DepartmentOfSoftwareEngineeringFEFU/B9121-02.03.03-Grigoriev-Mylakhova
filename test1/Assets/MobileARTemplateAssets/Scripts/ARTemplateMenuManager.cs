...
public class ARTemplateMenuManager : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Кнопка перехода к 3D-инструкции")]
    Button m_AssemblyButton;

    public Button assemblyButton
    {
        get => m_AssemblyButton;
        set => m_AssemblyButton = value;
    }
    ...
    void OnEnable()
    {
        ...
        m_AssemblyButton.onClick.AddListener(OnAssemblyButtonClicked);
    }
    ...
    void OnDisable()
    {
        ...
        m_AssemblyButton.onClick.RemoveListener(OnAssemblyButtonClicked);
    }
    ...
    void Update()
    {
        ...
        var focusedObj = m_InteractionGroup.focusInteractable?.transform.gameObject;
        if (focusedObj != null && focusedObj.GetComponent<AssemblyInfo>()?.hasAssembly == true)
            m_AssemblyButton.gameObject.SetActive(true);
        else
            m_AssemblyButton.gameObject.SetActive(false);
    ...
    /* void OnAssemblyButtonClicked()
     {
         var selectedObject = m_InteractionGroup.focusInteractable?.transform.gameObject;
         if (selectedObject == null) return;

         var info = selectedObject.GetComponent<AssemblyInfo>();
         if (info != null && info.hasAssembly)
         {
             GameObject prefab = info.assemblyPrefabOverride != null
                 ? info.assemblyPrefabOverride
                 : selectedObject;

             AssemblyData.prefabToAssemble = prefab;
    ...
    void OnAssemblyButtonClicked()
    {
        ...
        var info = selectedObject.GetComponent<AssemblyInfo>();
        if (info != null && info.hasAssembly && !string.IsNullOrEmpty(info.instructionSceneName))
        {
            UnityEngine.SceneManagement.SceneManager.LoadScene(info.instructionSceneName);
        }
        else
        {
            Debug.LogWarning("Объект не содержит корректной информации о сцене инструкции.");
        }
    }
